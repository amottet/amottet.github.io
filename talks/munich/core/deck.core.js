...
(function($, undefined) {
  ...
  var addNewSlideStates = function() {
    ...
    function togglePartnerDates(o) {
        for (index in o) {
            $('[data-tmdate="'+o[index]+'"]').each(function() {
    ...
    function disableAllDates() {
        $("[data-tmdate]").each(function() { $(this).removeClass("toggled");});
    }
      
    function getListOfDates(o) {
        if(o.data("tmdesc")) {
            var arrayDate = []
            if(typeof(o.data("tmdesc")) == "string") {
                var s = o.data("tmdesc").split(",");
                arrayDate = s.map(function (a) {return parseInt(a);});
            }
            else {
                arrayDate = [o.data("tmdesc")];
            }
            console.log(arrayDate);
            togglePartnerDates(arrayDate);
        }
        else {
            return [];
        }
    ...
    disableAllDates();
    if(currentIndex>=1) {
        $(".author-affiliation").css('opacity','1');
        $(".deck-status").css('opacity','1');
    }
    else {
        $(".author-affiliation").css('opacity','0');
        $(".deck-status").css('opacity','0');
    }
    if(slides[currentIndex].data("tmdesc")) {
        l = getListOfDates(slides[currentIndex]);
        togglePartnerDates(l);
    }
    slides[currentIndex].children().each(function() {
        l = getListOfDates($(this));
        togglePartnerDates(l);
    });
